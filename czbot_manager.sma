...
public plugin_init() {
    ...
    // Register CVars for persistence
    ...
    // Load saved settings
    g_CurrentMode = get_pcvar_num(g_pCvarMode);
    g_BotCount = get_pcvar_num(g_pCvarCount);
    g_BotDifficulty = get_pcvar_num(g_pCvarDifficulty);

    // Clamp values
    if (g_CurrentMode < 0 || g_CurrentMode > 3) g_CurrentMode = 0;
    if (g_BotCount < 0 || g_BotCount > 10) g_BotCount = 6;
    if (g_BotDifficulty < 0 || g_BotDifficulty > 3) g_BotDifficulty = 1;
...
public show_MainMenu(id) {
    new szTitle[128];
    formatex(szTitle, charsmax(szTitle), "\rCZ Bot Manager^n\wMode: \y%s^n\wBots: \y%d\w | Difficulty: \y%s",
    ...
    menu_additem(menu, "\yApply Changes Now", "5");
...
public show_ModeMenu(id) {
    new menu = menu_create("\rSelect Game Mode", "handler_ModeMenu");

    new szItem[64];
    for (new i = 0; i < 4; i++) {
        if (i == g_CurrentMode) {
            formatex(szItem, charsmax(szItem), "%s \r[Current]", g_szModeNames[i]);
        } else {
...
public show_DifficultyMenu(id) {
    new menu = menu_create("\rSelect Bot Difficulty", "handler_DifficultyMenu");

    new szItem[64];
    for (new i = 0; i < 4; i++) {
        if (i == g_BotDifficulty) {
            formatex(szItem, charsmax(szItem), "%s \r[Current]", g_szDifficultyNames[i]);
        } else {
...
public task_AddBots() {
    ...
    // Disable bot quota so we have full control
    ...
    switch (g_CurrentMode) {
        ...
        case MODE_MIXED_5V5: {
            ...
            // Add bots to both teams (5 each for 5v5)
            new iPerTeam = g_BotCount / 2;
            new iExtra = g_BotCount % 2;

            for (new i = 0; i < iPerTeam + iExtra; i++) {
                server_cmd("bot_add_ct");
            }
            for (new i = 0; i < iPerTeam; i++) {
                server_cmd("bot_add_t");
            }
